...
# `.. exec::module.path` renders a live Dash component. Its output exists
# ONLY in the React tree, so the machine lane, the prerender and the
# crawler HTML — all built from the markdown SOURCE — used to lose it
# entirely: the directive line was stripped and NOTHING replaced it.
# Measured on this fork 2026-08-31 (sync 1.6.42 note 80, muicharts):
# /showcase/robots-sandbox/llms.txt read "Move the switches. The document
# on the right is generated by..." followed by a blank line. The prose
# referred to a component the reader could not see. Four pages were
# affected. The `.. source::` treatment is the fix — expand the MODULE'S
# SOURCE, through the same fence-aware pass, so both consumers get one
# document from one parse.
...
def _expand_source_directives(markdown_content: str) -> str:
    ...
    """
    def exec_expansion(directive_line: str) -> str:
        """`.. exec::docs.pkg.mod` → that module's source, fenced.

        The component itself cannot be serialised into markdown, and a
        screenshot would be worse than nothing to an agent. Its SOURCE is
        the honest answer: it is what produces the demo the browser lane
        shows, it is the thing a reader would ask for next, and it keeps
        the two lanes describing the same artifact.
        """
        module = _EXEC_DIRECTIVE.match(directive_line).group(1).strip()
        rel = Path(module.replace('.', '/')).with_suffix('.py')
        try:
            content = rel.read_text()
            tail = '' if content.endswith('\n') else '\n'
            return (f'\n```python\n# Live component, rendered above on the '
                    f'browser lane.\n# Source: {rel}\n\n{content}{tail}```\n')
        except FileNotFoundError:
            return f'\n<!-- Error: exec module not found: {module} ({rel}) -->\n'
        except Exception as exc:
            return f'\n<!-- Error reading {rel}: {exc} -->\n'
