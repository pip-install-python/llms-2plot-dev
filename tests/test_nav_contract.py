...
def test_the_aside_collapses_on_pages_without_a_toc(app_module):
    ...
    # PORTED paths (DIVERGENCES.md 1): this fork deleted the template's
    # tutorial pages, so /backend-comparison does not exist here. Two of
    # this site's own `.. toc::` pages stand in for it.
    assert has_aside("/reference/configuration") and has_aside("/getting-started")
    for path in ("/changelog", "/", "/admin/traffic", "/api"):
        assert not has_aside(path), path
        assert aside_config(path)["collapsed"]["desktop"] is True
    assert aside_config("/reference/configuration")["collapsed"]["desktop"] is False
